...
int evristika()
{
	...
	if (number_of_sum == 2)
	{
		if ((digits_num[0][0] == digits_num[1][0]) && ((digits_num[7][0] - digits_num[0][0]) == 1))
		{
			if (digits[0][1] == digits[1][1])
			{
				...
				for (j = 1; j <= digits_num[7][0]; j++)
					if (j == 1)
						digits_num[7][j] = 1;
					else digits_num[7][j] = 0;
				return evr = 1;
			}
		}
		if (((digits_num[7][0] - digits_num[0][0]) == 1) && ((digits_num[7][0] - digits_num[1][0] == 1)))
		{
			if (digits[0][1] == digits[7][1])
			{
				digits_num[0][1] = 1;
				digits_num[7][1] = 1;
				digits_num[1][1] = 8;
...
int recurs(int slag, int digit)
{
	...
	if (evr == 0)
	{
		for (i = 0; i < number_of_sum; i++)
			...
			{
				if (j == 1) digits_num[i][j] = 1;
				else digits_num[i][j] = flag;
				srav(digits_num[i][j], i, j);
				if ((j != 1) && (flag == 0))flag = 2; else if (j != 1) flag++;
			}
		for (j = 1; j <= digits_num[7][0]; j++)
			if (j == 1) digits_num[7][j] = 1;
			else digits_num[7][j] = 0;
	}
...
int sum()
{
	...
	for (i = 0; i < number_of_sum; i++) {
		for (j = 1; j <= digits_num[i][0]; j++) a = a * 10 + digits_num[i][j];
		b += a;
		a = 0;
	}
	a = 0;
	for (j = 1; j <= digits_num[7][0]; j++) a = a * 10 + digits_num[7][j];
